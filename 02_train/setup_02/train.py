...
def train_until(iterations):
    ...
    gt_endo = gp.ArrayKey("GT_ENDO_AFFS")
    gt_lyso = gp.ArrayKey("GT_LYSO_AFFS")
    gt_endo_mask = gp.ArrayKey("GT_ENDO_AFFS_MASK")
    gt_lyso_mask = gp.ArrayKey("GT_LYSO_AFFS_MASK")
    ...
    add_affs = gp.AddAffinities(
            ...
            affinities=gt_endo,
            labels_mask=mask,
            affinities_mask=gt_endo_mask)
    add_affs += gp.AddAffinities(
            affinity_neighborhood=[[-1,0,0],[0,-1,0],[0,0,-1]],
            labels=labels_lyso,
            affinities=gt_lyso,
            labels_mask=mask,
            affinities_mask=gt_lyso_mask)
    ...
    balance_labels = gp.BalanceLabels(gt_endo, weights_endo, mask=gt_endo_mask)
    balance_labels += gp.BalanceLabels(gt_lyso, weights_lyso, mask=gt_lyso_mask)

    unsqueeze = gp.Unsqueeze([raw, gt_endo, gt_lyso, weights_endo, weights_lyso], axis=0)
    ...
    squeeze = gp.Squeeze([raw, gt_endo, gt_lyso, pred_endo, pred_lyso, weights_endo, weights_lyso], axis=0)
